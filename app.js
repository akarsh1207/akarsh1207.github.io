// Portfolio Website JavaScript - Fixed Navigation & Experience Scrolling + Image Loading
class PortfolioApp {
  constructor() {
  ...
  setupComponents() {
    ...
    this.setupImageFallbacks(); // NEW: Setup image loading fallbacks
  ...
  setupImageFallbacks() {
    ...
    const educationLogos = document.querySelectorAll('.education__logo-img');
    educationLogos.forEach((img, index) => {
      const fallback = img.nextElementSibling;
      if (fallback && fallback.classList.contains('education__logo-fallback')) {
        this.setupImageFallback(img, fallback);
      }
    });

    // Handle experience logos
    const experienceLogos = document.querySelectorAll('.experience__logo-img');
    experienceLogos.forEach((img, index) => {
      const fallback = img.nextElementSibling;
      if (fallback && fallback.classList.contains('experience__logo-fallback')) {
        this.setupImageFallback(img, fallback);
      }
    });
    ...
    if (profileImg) {
      ...
      if (fallback && fallback.classList.contains('profile-img-fallback')) {
        this.setupImageFallback(profileImg, fallback);
      }
    }
  ...
  setupImageFallback(img, fallback) {
    ...
    // Set up load handler to ensure image is visible when loaded
